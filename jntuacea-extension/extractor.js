(function () {
  if (window.__jnExtRun) return;
  window.__jnExtRun = true;
  ...
  function post(u, b) {
    ...
    return fetch(u, { method: 'POST',
      ...
      body: p.join('&'), credentials: 'same-origin' }).then(function (r) { return r.text(); });
  }
  ...
  function showSync(total) {
    ...
    o.innerHTML = '<div style="text-align:center;padding-top:70px">'
      ...
      + '<div id="jnStep" style="margin-top:22px;font-size:16px;font-weight:700;color:#1171e9">Processed 0 of ' + total + ' subjects  0%</div>'
      + '<div style="max-width:280px;margin:12px auto 0;height:8px;background:#e3ecf7;border-radius:6px;overflow:hidden">'
      + '<div id="jnBar" style="width:0%;height:100%;background:linear-gradient(90deg,#1171e9,#073d92);border-radius:6px;transition:width .3s"></div></div>'
      + '<div style="margin-top:26px;font-size:11px;color:#63728a">Secure session &middot; jntuaceastudents.classattendance.in</div></div>';
  }
  ...
  function showFail(msg) {
    ...
    o.innerHTML = '<div style="text-align:center;padding-top:80px">'
      ...
      + '<button onclick="document.getElementById(\'jnOverlay\').remove()" style="margin-top:18px;padding:10px 22px;border:none;border-radius:10px;background:#1171e9;color:#fff;font-size:14px;font-weight:700;cursor:pointer">Close</button></div>';
  }
  ...
  function showDash(name, roll, cls, out, totD, totP) {
    ...
    out.forEach(function (s) {
      ...
      cards += '<div style="background:#fff;border:1px solid #e3ecf7;border-radius:12px;padding:12px 14px;margin-bottom:8px;cursor:pointer" '
        + 'onclick="var l=this.querySelector(\'.jnlog\');if(l)l.style.display=l.style.display===\'block\'?\'none\':\'block\'">'
    ...
    o.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">'
      ...
      + '<div>'
      + '<button onclick="document.getElementById(\'jnOverlay\').remove()" style="border:none;background:#eef1f6;border-radius:8px;padding:6px 12px;font-size:13px;font-weight:700;color:#63728a;cursor:pointer;margin-right:6px">Close</button>'
      + '<button onclick="window.print()" style="border:none;background:#eef1f6;border-radius:8px;padding:6px 12px;font-size:13px;font-weight:700;color:#63728a;cursor:pointer">Print</button>'
      + '</div></div>'
  ...
  function runSync() {
    ...
    get('studenthome.php').then(function (home) {
      if (/This Page Does Not Exist/i.test(home)) {
        showFail('Login fine, but this portal copy does not host the attendance pages.');
        throw 'stop';
      }
      ...
      if (/This Page Does Not Exist/i.test(html)) {
        showFail('Login fine, but attendance pages are missing on this portal copy.');
        throw 'stop';
      }
      ...
      subjects.forEach(function (sub) {
        chain = chain.then(function () {
          ...
          return post('studentsubatt.php', sub).then(function (h) {
            ...
            if (!recs.length) {
              // attempt B: re-select for a fresh form, then submit
              var merged = {};
  ...
  function attachButton() {
    ...
    if (document.getElementById('jnFloatingBtn')) return;
